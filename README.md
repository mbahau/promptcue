...
click once into the o9 chat box, then just
...
you stay in the o9 chat the whole time.
...
6. **Choose delivery style:** the **"Simulate typing"** checkbox controls how
   the prompt appears in the o9 chat box:
   ...
   - **Unchecked**: the prompt is pasted instantly (old behavior) — use this
     if simulated typing ever misbehaves in the o9 chat box (see notes
     below).
...
7. **Run the demo:**
   - Click into the **o9 chat input box** (not the app window) so it has
     keyboard focus.
   - Press **F2** → the next prompt is delivered directly into the o9 chat
...
- When **simulating typing**, a line break inside a prompt is sent as
  **Shift+Enter** (the common "new line without submitting" shortcut in most
  chat boxes). If the o9 chat box uses a different shortcut for that, a
...
- Simulated typing sends real keystrokes one at a time into the o9 chat box.
  ...
  once against the actual o9 chat box before using it live. If it's ever
